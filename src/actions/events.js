...
export const getLandingEvents = () => async (dispatch) => {
	try {
		const res = await axios.get(`${baseUrl}/api/events/landing`);
		dispatch({
...
export const getEvent = (id) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.get(`${baseUrl}/api/events/event/${id}`, config);
		dispatch({
			type: GET_EVENT,
			payload: res.data,
		});
...
export const unRegisterForEvent = (id) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.post(
			`${baseUrl}/api/event/unregister/${id}`,
			config
		);
		dispatch({
			type: UPDATE_EVENT_REGISTRATION,
...
export const likeEvent = (id) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.post(`${baseUrl}/api/events/like/${id}`, config);
		dispatch({
...
export const unlikeEvent = (id) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.post(`${baseUrl}/api/events/unlike/${id}`, config);
		dispatch({
			type: UPDATE_EVENT_LIKES,
			payload: {
				id,
...
export const deleteEventComment = (eventId, commentId) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.delete(
			`${baseUrl}/api/events/deletecomment/${eventId}/${commentId}`,
			config
		);
		if (res.status === 200) {
			dispatch({
