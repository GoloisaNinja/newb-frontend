...
export const getProfile = (id) => async (dispatch) => {
	try {
		const res = await axios.get(`${baseUrl}/api/profiles/${id}`);
		dispatch({
			type: GET_PROFILE,
			payload: res.data,
		});
...
export const createProfile =
	...
	async (dispatch) => {
		...
		try {
			...
			const body = formData;
			const res = await axios.post(`${baseUrl}/api/profiles`, body, config);
			dispatch({
				type: CREATE_PROFILE,
				payload: res.data,
			});
			dispatch(
				setAlert(
					edit ? 'Changed your deets' : 'Created your profile',
					'success'
				)
			);
