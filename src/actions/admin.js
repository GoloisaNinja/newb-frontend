...
export const adminGetAllUsers = () => async (dispatch) => {
	...
	try {
		const res = await axios.get(`${baseUrl}/api/users/admin/getAll`, config);
		dispatch({
...
export const adminDeleteUserAvatar = (userId) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.delete(
			`${baseUrl}/api/users/admin/avatar/delete/${userId}`,
			config
		);
		dispatch({
...
export const adminDeleteUser = (userId) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.delete(
			`${baseUrl}/api/users/admin/user/delete/${userId}`,
			config
		);
		dispatch({
...
export const adminEditAdmin = (userId, editParam) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.patch(
			`${baseUrl}/api/users/admin/user/isAdmin/${userId}`,
			body,
			config
		);
		if (res.status === 200) {
...
export const adminDeleteObstacle = (obstacleId) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.delete(
			`${baseUrl}/api/obstacles/admin/delete/${obstacleId}`,
			config
		);
		dispatch({
...
export const adminGetObstacleById = (obstacleId) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.get(
			`${baseUrl}/api/obstacles/admin/obstacle/${obstacleId}`,
			config
		);
		dispatch({
...
export const adminEditObstacle =
	(formData, obstacleId, history) => async (dispatch) => {
		const token = localStorage.getItem('token');
		try {
			const config = {
				headers: {
			...
			const res = await axios.post(
				`${baseUrl}/api/obstacles/admin/edit/${obstacleId}`,
				body,
				config
			);
			if (res.status === 200) {
...
export const adminAddObstacle = (formData) => async (dispatch) => {
	...
	try {
		...
		const body = formData;
		const res = await axios.post(
			`${baseUrl}/api/obstacles/admin/create`,
			body,
			config
		);
		if (res.status === 200) {
...
export const adminGetAllWorkouts = () => async (dispatch) => {
	try {
		const res = await axios.get(`${baseUrl}/api/workouts/all`);
		dispatch({
...
export const adminDeletePost = (postId) => async (dispatch) => {
	...
	try {
		...
		const res = await axios.delete(
			`${baseUrl}/api/posts/admin/delete/${postId}`,
			config
		);
		dispatch({
...
export const adminAddEvent = (formData) => async (dispatch) => {
	...
	try {
		...
		const body = formData;
		const res = await axios.post(`${baseUrl}/api/events/create`, body, config);
		if (res.status === 200) {
...
export const adminEditEvent =
	(formData, eventId, history) => async (dispatch) => {
		const token = localStorage.getItem('token');
		try {
			const config = {
				headers: {
			...
			const body = formData;
			const res = await axios.post(
				`${baseUrl}/api/events/admin/edit/${eventId}`,
				body,
				config
			);
			if (res.status === 200) {
